...
/**
 * Skill Adapter (PROJ-020)
 *
 * Converts canonical skill format to platform-specific formats
 * for all 7 frontends: claude, opencode, gemini, pi, wocoder, antigravity, codex.
 *
 * Usage:
  ...
  case "generate": {
    ...
    for (const skill of targetSkills) {
      ...
      for (const platform of targetPlatforms) {
        if (platform === "codex") {
          const codexDir = join(ROOT, "codex-output");
          const files = await generateForPlatform(skill, platform, codexDir);
          console.log(`  ✓ ${skill.name} → codex-output/${platform}/skills/${skill.name}/ (${files.length} files)`);
          total += files.length;
          continue;
        }
