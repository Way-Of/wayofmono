...
## GDPR Compliance Requirements (Per WOW-063)

OptiCat must comply with GDPR as it processes personal data (technician names, service reports with customer info, project locations, etc.):

- [ ] **Right to erasure ("right to be forgotten")**: Per-user data deletion API — when a technician leaves, their `owner_user_id`/`assigned_user_id` references must be anonymized or reassigned, not just deleted
- [ ] **Data portability**: Export endpoint `GET /api/opticat/export/user/:userId` → JSON with all projects, aggregat, reports assigned to that user
- [ ] **Data minimization**: Audit OptiCat tables for unnecessary personal data fields; remove any non-essential PII
- [ ] **Access logs**: All OptiCat data access (read/write) logged to `audit_logs` with `userId`, `tenantId`, `resource_type`, `action`, `timestamp`
- [ ] **Breach notification**: Per-tenant DB isolation (WOW-063) enables identifying affected tenants; OptiCat must log which tenant DB was accessed
- [ ] **Storage limitation**: Configurable retention policy per tenant for service reports (e.g., auto-archive after 7 years, delete after 10)
- [ ] **Cross-border data transfer**: OptiCat Flutter backend (Fly.io) must deploy tenant DBs in correct region (EU vs non-EU)

### Per-Tenant Database Architecture (Per WOW-063)

OptiCat MUST use the `getDb(tenantId)` pattern from `server/db.ts`:

- [ ] **Migration**: `plugin/opticat/server/api.ts` imports `getDb` (not `db` singleton)
- [ ] Every query uses `getDb(auth.tenantId).query(...)` — no cross-tenant access possible
- [ ] OptiCat tables created in per-tenant DB via `initializeTenantSchema()` in `server/db.ts:65-78`
- [ ] Flutter backend (`chat_server/`) connects to same per-tenant SQLite file OR uses WoW API proxy (recommended)
- [ ] VNC sessions isolated per tenant+user via `server/vnc-display-manager.ts` (WOW-095)

### Per-User Isolation Within Per-Tenant DB

Since each tenant has their own SQLite file, user-level isolation is implemented via:

1. **Row-level filters** in all queries (see Scope section 2)
2. **Assignment join tables** (`opticat_project_assignments`, `opticat_aggregat_assignments`)
3. **Capability checks** via `hasCapability(auth.role, 'opticat:view_projects')` — admins bypass user filters
4. **Audit logging** with `userId` for every mutation

## Dependencies
- **WOW-096** (tenant isolation) — must be complete first
- **WOW-095** (multi-user concurrency) — for concurrent user sessions
- **WOW-063** (tenant-data-isolation-architecture) — base `getDb` pattern, per-tenant DB, GDPR framework
...
- **Flutter backend should proxy via WoW API** (`/api/opticat/*`) rather than direct SQLite access — ensures single source of truth for auth/tenant/isolation
