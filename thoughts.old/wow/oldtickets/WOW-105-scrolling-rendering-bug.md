# WOW-105: Critical Bug: Persistent Missing Scrollbar & Rendering Failure on Overview Page

## Problem
The System Overview Page (`/overview`) is currently experiencing a critical rendering failure (page is not visible) and persistent missing scrollbars in both the browser and Electron environments, despite multiple layout refactor attempts.

## Symptoms
- Page fails to render content in the browser (`http://localhost:5173/overview`).
- Scrollbars do not appear when content exceeds viewport height.
- Attempted fixes (`overflow-y-scroll`, layout refactoring) have caused regressions (page visibility failure).

## Priority
Critical - Blocks access to the System Overview/Landing Page and fundamental navigation.

## Scope
- Investigate and fix the rendering failure.
- Ensure a reliable, always-visible scrollbar mechanism in both browser and Electron.
- Clean up the layout hierarchy in `App.tsx` and `SystemOverviewPage.tsx` to adhere to WoW structural standards.
...
- `index.css`: `html { scrollbar-color: ... }` with implicit height constraints
...
2. **`src/pages/SystemOverviewPage.tsx` (line 8)**: Changed `min-h-full` → `min-h-screen`
   - Ensures page stretches to at least viewport height
...
6. **Removed broken component**: `src/components/UpdateNotification.tsx`
...
Scrolling works on both ports. Public pages have independent scroll context via `PublicLayout`. Authenticated workspace retains locked layout via `App.tsx`.

## Next Steps
1. Test in Electron (`bun run electron:dev` - kill port 3333 first)
2. Verify other public pages (`/login`, `/welcome`, `/setup`) scroll correctly
3. Ensure authenticated routes still work with locked layout

---

## Additional Updates Applied (2026-06-07)
