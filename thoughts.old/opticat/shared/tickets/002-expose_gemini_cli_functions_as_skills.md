...
**Goal**: Create a basic Dart Frog server capable of receiving HTTP requests.
**Location**: A new directory, e.g., `gemini_cli_skills_server/` at the project root.

**File**: `gemini_cli_skills_server/pubspec.yaml`
**Changes**: Initialize a new Dart Frog project and add necessary dependencies (e.g., `dart_frog`, `shelf_router`, `shelf_cors_headers`).
...
- [ ] Dart Frog server starts successfully: `cd gemini_cli_skills_server && dart_frog dev` (or similar command).
