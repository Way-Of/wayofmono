...
## v0.4.32 (2026-06-17) — Structure-Agnostic Developer Discovery + Profile/Settings + Login Fixes
...
- **Auto-redirect timing bug**: GitHub OAuth callback now uses `setFromSession` instead of `login` — fixes race condition where developers array wasn't loaded yet, leaving users stuck on login page
- **Login matching**: `login()` now matches by both `githubUsername` and `id` (directory name)
...
### New Features
- **Profile page** (`/profile`): Avatar, display name, GitHub username, role badge, project tags, account details, pincode change form, GitHub connection status
- **Settings page** (`/settings`): Data source toggle (GitHub/Local), repository/branch config, cache TTL display + manual refresh, app info, config file path
- **Login page toggle**: Tab to switch between GitHub OAuth and pincode login modes
- **Version in logo**: `wodev` banner now shows `v0.4.32  port 6969`
- **Clickable user section**: Click avatar/name in sidebar → opens Profile

### Centralized Config System (new)
- **`ui/src/lib/env.ts`**: Tiered config — defaults → `~/.config/wodev/config.json` → env vars
- **Configurable options**: `GITHUB_REPO`, `GITHUB_BRANCH`, `THOUGHTS_ROOT`, `WODEV_PROJECTS`, `WODEV_SKIP_DIRS`, `PORT`, `GITHUB_CACHE_TTL`
- **JSON config file**: `~/.config/wodev/config.json` with full settings
- **Env var overrides**: All settings can be overridden via `process.env`
