...
async function printOnboarding(format: "text" | "json" | "markdown"): Promise<void> {
  if (format === "json") {
    console.log(JSON.stringify({
      steps: [
        ...
        "Pick your tool(s): claude, opencode, pi, wocode, antigravity, codex",
  ...
  if (format === "markdown") {
    ...
    console.log("Supported tools: claude, opencode, pi, wocode, antigravity, codex\n");
    console.log("```bash");
  ...
  console.log(`   Supported: claude, opencode, pi, wocode, antigravity, codex`);
  console.log(`   ai-harness --tool=claude`);
