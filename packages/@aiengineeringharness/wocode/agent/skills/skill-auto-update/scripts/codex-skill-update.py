#!/usr/bin/env python3
"""
Codex CLI Skill Updater — codex/skills

Ensures all skills use correct:
  - Directory naming: snake_case
  - name field: snake_case
  - allowed-tools casing: lowercase_snake
  - allowed-tools format: yaml_list
  - Frontmatter fields: name, description, tools, model, temperature, version

Usage:
    python3 scripts/codex-skill-update.py --validate
    python3 scripts/codex-skill-update.py --fix
    python3 scripts/codex-skill-update.py --add <name> --desc "..."
    python3 scripts/codex-skill-update.py --sync-yaml
    python3 scripts/codex-skill-update.py --all
"""

import os, sys, re, json, yaml

REPO_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
SKILL_DIR = os.path.join(REPO_ROOT, "codex/skills")
CONFIG_YAML = os.path.join(REPO_ROOT, "config-manifest/tools/codex.yaml")
...
TARGET_TOOLS_CASE = "lowercase_snake"
TOOLS_FMT = "yaml_list"
SUPPORTED_FIELDS = ['name', 'description', 'tools', 'model', 'temperature', 'version']
HAS_COMMANDS = False
COMMAND_DIR = None
TOOLS_EXAMPLE = "[read_file, write_file, run_shell_command]"
...
def print_help():
    print("Codex CLI Skill Updater")
    print("Usage:")
    print(f"  python3 scripts/codex-skill-update.py --validate    Validate all skills")
    print(f"  python3 scripts/codex-skill-update.py --fix         Auto-fix issues")
    print(f"  python3 scripts/codex-skill-update.py --add NAME    Scaffold new skill")
    print(f"  python3 scripts/codex-skill-update.py --sync-yaml   Sync YAML config")
    print(f"  python3 scripts/codex-skill-update.py --all         Fix + sync-yaml")
