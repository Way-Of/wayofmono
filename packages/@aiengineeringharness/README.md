...
./setup.sh wocoder            # Install Wo Coder → ~/.wocoder/
./setup.sh all                # Install all five
...
Specialized sub-agents shared across Claude Code, OpenCode, Gemini CLI, and Wo Coder:
...
All commands work identically across tools. OpenCode uses `commands/` + `skills/`, Claude Code uses `skills/` (commands have `disable-model-invocation: true`), Gemini CLI uses TOML format, Pi uses `prompts/` (prompt templates) + `skills/`, and Wo Coder uses `commands/` + `skills/`.
...
Works for all five tools — Claude Code calls them "subagents", OpenCode, Gemini CLI, and Wo Coder call them "agents", and Pi calls them "agents" (kebab-case).
...
- **Wo Coder**: Add `.md` files in `wocoder/commands/` or `wocoder/skills/<name>/SKILL.md`
