...
description: >-
  Validate all skill SKILL.md files across all 7 tool harnesses for correct
  frontmatter, naming, allowed-tools casing, and format compliance. Use when the
  user asks to check compliance, validate skills, run compliance check, or
  verify skill formatting.
allowed-tools: 'read, file, write, file, run, shell, command'
---
...
```bash
deno run -A packages/@aiengineeringharness/scripts/compliance-check.ts --tool=codex
```

Replace `codex` with: `opencode`, `claude`, `gemini`, `pi`, `antigravity`, `wocoder`.
