...
name: worktree
description: >-
  Manage git worktrees for parallel development workflows integrated with the
  harness ticket system. Use when the user mentions worktrees, wants to work on
  multiple branches simultaneously, asks about parallel development, or wants to
  create an isolated workspace for a ticket or feature. Uses run_shell_command
  for git operations.
allowed-tools: 'read, file, run, shell, command, glob, search, file, content'
---
...
- Mentions "worktree(s)" in any context
- Wants to "work on this in parallel" or "work on multiple things"
- Asks for an "isolated workspace" or "separate workspace"
- Says "clean up worktrees" or "what am I working on"

**Do NOT activate** when:
- User is doing normal branch switching (`git checkout`, `git switch`)
- No indication they need parallel workspaces
...
Parse the first argument. If none provided or intent is unclear, print:
