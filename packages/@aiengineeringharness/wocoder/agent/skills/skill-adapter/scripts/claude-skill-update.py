#!/usr/bin/env python3
"""
Claude Code Skill Updater — claude/skills

Ensures all skills use correct:
  - Directory naming: snake_case
  - name field: snake_case
  - allowed-tools casing: PascalCase
  - allowed-tools format: space_string
  - Frontmatter fields: name, description, allowed-tools, disable-model-invocation

Usage:
    python3 scripts/claude-skill-update.py --validate
    python3 scripts/claude-skill-update.py --fix
    python3 scripts/claude-skill-update.py --add <name> --desc "..."
    python3 scripts/claude-skill-update.py --sync-yaml
    python3 scripts/claude-skill-update.py --all
"""

import os, sys, re, json, yaml

REPO_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
SKILL_DIR = os.path.join(REPO_ROOT, "claude/skills")
CONFIG_YAML = os.path.join(REPO_ROOT, "config-manifest/tools/claude.yaml")
...
TARGET_TOOLS_CASE = "PascalCase"
TOOLS_FMT = "space_string"
SUPPORTED_FIELDS = ['name', 'description', 'allowed-tools', 'disable-model-invocation']
HAS_COMMANDS = True
COMMAND_DIR = None
TOOLS_EXAMPLE = "Read, Write, Edit, Bash"
...
def print_help():
    print("Claude Code Skill Updater")
    print("Usage:")
    print(f"  python3 scripts/claude-skill-update.py --validate    Validate all skills")
    print(f"  python3 scripts/claude-skill-update.py --fix         Auto-fix issues")
    print(f"  python3 scripts/claude-skill-update.py --add NAME    Scaffold new skill")
    print(f"  python3 scripts/claude-skill-update.py --sync-yaml   Sync YAML config")
    print(f"  python3 scripts/claude-skill-update.py --all         Fix + sync-yaml")
