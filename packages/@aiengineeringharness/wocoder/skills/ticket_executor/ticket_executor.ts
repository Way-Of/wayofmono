// Auto-generated from canonical skill: ticket_executor
// Platform: WoCoder (Node/Deno)
// Description: Processes tickets sequentially (one at a time) and verifies work completion in the codebase. Uses sequential processing with verification after each ticket.

export const skill = {
  name: "ticket_executor",
  description: "Processes tickets sequentially (one at a time) and verifies work completion in the codebase after each ticket. Never processes multiple tickets simultaneously. Verifies completion against the codebase, reports progress, blocks next ticket until current is verified.",
  tools: ["read", "grep", "glob", "find", "ls", "tree", "git", "npm", "make"],
  prompt: `> **Platform**: WoCoder | **Skill**: ticket-executor | **Version**: 1.0.0
>
...
>   - Code changes in git (commit/PR merged)
>   - Tests passing
>   - Documentation updated
>   - Acceptance criteria met
>   - Production-ready standard (no mocks, error handling, observability, security)
...
> 2. **Sequential Load**: Load one ticket at a time from \`thoughts/<project>/shared/tickets/<ID>.md\`
> 3. **Verify Codebase**: Check acceptance criteria:
>    - Files changed in repository?
>    - Tests passing?
>    - PR merged?
>    - Documentation updated?
> 4. **Block Next**: Do NOT proceed to next ticket until current is verified
