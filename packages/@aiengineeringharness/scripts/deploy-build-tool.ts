...
const TOOLS: Record<string, ToolSpec> = {
  opencode: { name: "build_tool", tools: "read, write, bash, edit, grep, glob, webfetch, websearch, question", naming: "snake_case" },
  claude: { name: "build_tool", tools: "Read, Write, Bash, Edit, Glob, Grep, WebFetch, WebSearch", naming: "snake_case" },
  gemini: { name: "build_tool", tools: "read, write, bash, glob, grep, web, code", naming: "snake_case" },
...
for (const [tool, spec] of Object.entries(TOOLS)) {
  const dirName = spec.naming === "kebab-case" ? "build-tool" : "build_tool";
  const dir = `${HARNESS_ROOT}/${tool}/skills/${dirName}`;
  await ensureDir(dir);

  const content = CANONICAL
    .replaceAll("TOOL_NAME", spec.name)
    .replaceAll("TOOL_TOOLS", spec.tools);

  Deno.writeTextFileSync(`${dir}/SKILL.md`, content);
  console.log(`  ${tool}/${dirName}: written ✅`);
}

console.log("\nDone! build_tool deployed to all 7 harness dirs.");
