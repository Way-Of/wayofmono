...
> [!NOTE]
> This reference guide is based on the official [Antigravity/Gemini Plugins Documentation](https://antigravity.google/docs/plugins).

Plugins are namespaced bundles that allow you to extend Gemini's capabilities by grouping skills, rules, MCP servers, and hooks into a single package.
...
Every plugin must have a `plugin.json` file at its root. This file identifies the directory as a Gemini plugin.
...
Gemini supports two methods for installing and loading plugins:

### 1. Using Google-Bundled Plugins (Build with Google)
Gemini comes with pre-packaged plugins. You can browse and add these directly from the user interface:
...
Place your plugin folder in one of the designated customization directories. Gemini automatically scans and loads them on startup:
