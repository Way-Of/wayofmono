#!/usr/bin/env python3
"""
Pi Skill Updater — pi/agent/skills

Ensures all skills use correct:
  - Directory naming: kebab-case
  - name field: kebab_case
  - allowed-tools casing: lowercase
  - allowed-tools format: yaml_list
  - Frontmatter fields: name, description, allowed-tools

Usage:
    python3 scripts/pi-skill-update.py --validate
    python3 scripts/pi-skill-update.py --fix
    python3 scripts/pi-skill-update.py --add <name> --desc "..."
    python3 scripts/pi-skill-update.py --sync-yaml
    python3 scripts/pi-skill-update.py --all
"""

import os, sys, re, json, yaml

REPO_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
SKILL_DIR = os.path.join(REPO_ROOT, "pi/agent/skills")
CONFIG_YAML = os.path.join(REPO_ROOT, "config-manifest/tools/pi.yaml")
...
def print_help():
    print("Pi Skill Updater")
    print("Usage:")
    print(f"  python3 scripts/pi-skill-update.py --validate    Validate all skills")
    print(f"  python3 scripts/pi-skill-update.py --fix         Auto-fix issues")
    print(f"  python3 scripts/pi-skill-update.py --add NAME    Scaffold new skill")
    print(f"  python3 scripts/pi-skill-update.py --sync-yaml   Sync YAML config")
    print(f"  python3 scripts/pi-skill-update.py --all         Fix + sync-yaml")
