...
const CONFIG_PATH = join(homedir(), ".wo", "agent", "web-search.json");
...
export async function search(query: string, options: FullSearchOptions = {}): Promise<AttributedSearchResponse> {
	...
	if (provider === "gemini") {
		...
		throw new Error(
			...
			"  1. Set GEMINI_API_KEY in ~/.wo/agent/web-search.json\n" +
	...
	throw new Error(
		...
		"  1. Set perplexityApiKey in ~/.wo/agent/web-search.json\n" +
		"  2. Set EXA_API_KEY (or exaApiKey) in ~/.wo/agent/web-search.json\n" +
		"  3. Set GEMINI_API_KEY in ~/.wo/agent/web-search.json\n" +
