...
```bash
# From the monorepo
pnpm add @wayofmono/web-access

# Or from npm
npm install @wayofmono/web-access
```

Works immediately with no API keys — Exa MCP provides zero-config search. For more providers or direct API access, add keys to `~/.wo/agent/web-search.json`:
...
Bundled research workflow for investigating open-source libraries. Combines GitHub cloning, web search, and git operations (blame, log, show) to produce evidence-backed answers with permalinks. Wo loads it automatically based on your prompt.
...
Persists to `~/.wo/agent/web-search.json` and takes effect on the next `web_search` call. When disabled, `web_search` returns raw results without opening the curator window.
...
All config lives in `~/.wo/agent/web-search.json`. Every field is optional.
...
Both shortcuts are configurable via `~/.wo/agent/web-search.json`:
...
Values use the same format as Wo keybindings (e.g. `ctrl+s`, `ctrl+shift+s`, `alt+r`). Changes take effect on next agent restart.

Set `"enabled": false` under any feature to disable it. Config changes require an agent restart.
