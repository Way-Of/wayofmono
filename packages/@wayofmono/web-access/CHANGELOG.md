...
### Added
- Added `summaryModel` config for choosing the default curator summary draft model from `~/.wo/web-search.json`.

### Fixed
- Made Gemini Web browser-cookie access opt-in via `allowBrowserCookies` or `WO_ALLOW_BROWSER_COOKIES=1`, preventing surprise macOS Keychain prompts during provider checks.
...
- Migrated extension tool schemas from `@sinclair/typebox` to wo's bundled `typebox` 1.x import path.

## [0.10.6] - 2026-04-04

### Changed
- Added `promptSnippet` metadata for `web_search`, `code_search`, `fetch_content`, and `get_search_content` so wo 0.59+ includes these tools in the default prompt tool section and improves discoverability of research/fetch flows.
...
- **`/curator` command.** Toggle or configure the curator workflow at runtime: `/curator on`, `/curator off`, `/curator summary-review`, or `/curator` to toggle. Persists to `~/.wo/web-search.json` and takes effect on the next `web_search` call.
- **Config-based workflow default.** Added `workflow` field to `~/.wo/web-search.json` for persistent curator preference. Per-call `workflow` parameter on `web_search` takes priority, then config, then the built-in default (`summary-review` with UI, `none` without).
...
- Fixed error-signal downgrade in local-video API fallback (`video-extract.ts`): `tryVideoGeminiApi` now rethrows config parse failures (`Failed to parse ~/.wo/web-search.json`) instead of swallowing them as `null`, preserving actionable root-cause errors.
- Fixed provider config type hardening in search routing (`index.ts`): `normalizeProviderInput` now guards non-string config values before trimming so malformed `provider` entries in `~/.wo/web-search.json` no longer crash runtime provider resolution with `value.trim is not a function`.
...
- Updated curator session timeout defaults in `index.ts`: curator now starts at 20 seconds by default (down from 60) and can be configured via `curatorTimeoutSeconds` in `~/.wo/web-search.json` (capped at 600 seconds).
- Hardened `/websearch` startup error handling in `index.ts`: config/provider bootstrap now runs behind explicit error handling so malformed `~/.wo/web-search.json` no longer throws uncaught command errors before the existing server-start try/catch; users now receive a direct UI error with parse context.
- Hardened extension bootstrap config handling in `index.ts`: shortcut initialization now uses guarded config loading, logging parse errors and falling back to default shortcuts instead of crashing extension registration on malformed `~/.wo/web-search.json`.
...
- **Auto-condense pipeline.** When the countdown expires without manual curation, a single LLM call (Claude Haiku by default) condenses all search results into a deduplicated briefing organized by topic. Preprocessing enriches the prompt with URL overlap, answer similarity, and source quality analysis. Configure via `"autoFilter"` in `~/.wo/web-search.json`. Full uncondensed results stored and retrievable via `get_search_content`.
- **Configurable keyboard shortcuts.** Both shortcuts (curate: Ctrl+Shift+S, activity monitor: Ctrl+Shift+W) can be remapped via `"shortcuts"` in `~/.wo/web-search.json`. Changes take effect on restart.
- **`/websearch` command** — opens the curator directly from wo without an agent round-trip. Accepts optional comma-separated queries or opens empty.
...
- **Provider selection** — global dropdown in the curator UI to switch between Perplexity and Gemini. Persists to `~/.wo/web-search.json`.
...
- `search-filter.ts` — auto-condense pipeline: preprocessing, LLM condensation via wo's model registry, and post-processing (citation verification, source list completion).
...
- `package.json` now includes `repository`, `homepage`, `bugs`, and `description` fields (repo link was missing from wo packages site)
...
- `searchProvider` config option in `~/.wo/web-search.json` for global provider default
...
- YouTube video understanding in `fetch_content` via three-tier fallback chain:
  ...
  - **Gemini API** (secondary): direct REST calls with `GEMINI_API_KEY`. YouTube URLs passed as `file_data.file_uri`. Configure via `GEMINI_API_KEY` env var or `geminiApiKey` in `~/.wo/web-search.json`.
...
- Configurable via `~/.wo/web-search.json` under `youtube` key (`enabled`, `preferredModel`)
...
### Added
- GitHub repository clone extraction for `fetch_content` -- detects GitHub code URLs, clones repos to `/tmp/wo-github-repos/`, and returns actual file contents plus local path for further exploration with `read` and `bash`
...
- Configurable via `~/.wo/web-search.json` under `githubClone` key (enabled, maxRepoSizeMB, cloneTimeoutSeconds, clonePath)
...
### Fixed
- Adapt execute signatures to wo v0.51.0: reorder signal, onUpdate, ctx parameters across all three tools
...
### Changed
- Added `wo` manifest to package.json for wo v0.50.0 package system compliance
- Added `wo-package` keyword for npm discoverability
...
Initial release. Designed for wo v0.37.3.
...
- Config file support (`~/.wo/web-search.json`)
