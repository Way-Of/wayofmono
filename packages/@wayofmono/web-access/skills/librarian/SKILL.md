...
Wo executes tool calls sequentially, even when you emit multiple calls in one turn. But batching independent calls in a single turn still saves LLM round-trips (~5-10s each). Use these patterns:
...
4. Get the commit SHA: `cd /tmp/wo-github-repos/owner/repo && git rev-parse HEAD`
...
```bash
cd /tmp/wo-github-repos/owner/repo
...
```bash
cd /tmp/wo-github-repos/owner/repo && git rev-parse HEAD
```
...
| gh CLI rate limited | Use the already-cloned repo in /tmp/wo-github-repos/ for git operations |
