...
Persists to `~/.wo/web-search.json` and takes effect on the next `web_search` call. When disabled, `web_search` returns raw results without opening the curator window.
...
All config lives in `~/.wo/web-search.json`. Every field is optional.
...
Both shortcuts are configurable via `~/.wo/web-search.json`:
