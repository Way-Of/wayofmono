...
export function getSelfUpdateUnavailableInstruction(
	...
	if (method === "bun-binary") {
		return `Download from: https://github.com/Way-Of/wayofmono/releases/latest`;
	}
	const command = getSelfUpdateCommandForMethod(method, packageName, updatePackageName, npmCommand);
	if (command) {
...
export function getPackageDir(): string {
	...
	const envDir = process.env.WO_PACKAGE_DIR;
	if (envDir) {
...
export const PACKAGE_NAME: string = pkg.name || "@wayofmono/wo-agent";
export const APP_NAME: string = "wouser";
export const APP_TITLE: string = "WayOfMono User Agent";
...
const DEFAULT_SHARE_VIEWER_URL = "https://wo.dev/session/";

/** Get the share viewer URL for a gist ID */
export function getShareViewerUrl(gistId: string): string {
	const baseUrl = process.env.WO_SHARE_VIEWER_URL || DEFAULT_SHARE_VIEWER_URL;
