...
type SkillDiscoveryMode = "wouser" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "wouser" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readWoManifestFile(packageJsonPath: string): WoManifest | null {
	try {
		...
		const pkg = JSON.parse(content) as { wouser?: WoManifest };
		return pkg.wouser ?? null;
	} catch {
		return null;
	}
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "wouser");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private addAutoDiscoveredResources(
		...
		// Project skills from .wo/
		addResources(
			"skills",
			collectAutoSkillEntries(projectDirs.skills, "wouser"),
			projectMetadata,
		...
		// User skills from ~/.wo/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "wouser"),
			userMetadata,
