...
function isOfflineModeEnabled(): boolean {
	const value = process.env.WO_OFFLINE;
...
type SkillDiscoveryMode = "wocode" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "wocode" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readWoManifestFile(packageJsonPath: string): WoManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { wocode?: WoManifest };
		return pkg.wocode ?? null;
	} catch {
		return null;
	}
}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readWoManifestFile(packageJsonPath);
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "wocode");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "wo-extensions", private: true };
	...
	private getTemporaryDir(prefix: string, suffix?: string): string {
		const hash = createHash("sha256")
			...
			.digest("hex")
			.slice(0, 8);
		return join(tmpdir(), "wo-extensions", prefix, hash, suffix ?? "");
	}
	...
	private collectPackageResources(
		...
		const manifest = this.readWoManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof WoManifest];
	...
	private readWoManifest(packageRoot: string): WoManifest | null {
	...
	private addAutoDiscoveredResources(
		...
		// Project extensions from .wocode/
		addResources(
			"extensions",
		...
		// Project skills from .wocode/
		addResources(
			"skills",
			collectAutoSkillEntries(projectDirs.skills, "wocode"),
			projectMetadata,
		...
		// User extensions from ~/.wocode/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.wocode/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "wocode"),
			userMetadata,
