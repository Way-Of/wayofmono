...
> [!NOTE]
> This reference guide is based on the official [Antigravity/Gemini Hooks Documentation](https://antigravity.google/docs/hooks).

Hooks are interceptors that allow you to monitor, inspect, and modify the behavior of a Gemini agent at specific stages of its execution lifecycle. Hooks are essential for enforcing custom rules, running linters, logging diagnostic traces, and establishing security guardrails (e.g., blocking forbidden commands or sensitive credentials).
...
When writing programmatic hooks using the SDK, hooks are divided into three functional categories:
